...
async function exportStylusVerifier(zKeyName, template, logger) {
    const verificationKey = await zkeyExportVerificationKey(zKeyName, logger);
...
const commands = [
    ...
    {
        cmd: "groth16 export stylusverifier [circuit_final.zkey] [output_dir]",
        description: "Creates a verifier in stylus",
        alias: ["g16esv"],
        action: groth16ExportStylusVerifier
    },
    {
...
async function groth16ExportStylusVerifier(params, options) {
    let zkeyName;
    let verifierOutput;
    ...
    if (params.length < 2) {
        ...
        verifierOutput = params[1];
    }

    if (options.verbose) Logger__default["default"].setLogLevel("DEBUG");
    
    fs__default["default"].cpSync("templates/groth16_verifier_template", verifierOutput, {recursive: true});
    ...
    const verifierCode = await exportStylusVerifier(zkeyName, template, logger);

    fs__default["default"].writeFileSync(verifierOutput+"/src/lib.rs", verifierCode, "utf-8");
